...
export async function rebuildDeterministicMoc(scope: MocMaintenanceScope = "all"): Promise<MocMaintenanceResult> {
  ...
  const repository = new FilesystemArticleRepository(config.paths.vaultRoot); const service = new IndexService(config.paths.vaultRoot, repository); const manifest = await service.rebuildMoc(); return { scope, publicArticleCount: manifest.public_article_count, generatedDir: service.generatedDir, dailyDeferred: scope === "all" };
}
