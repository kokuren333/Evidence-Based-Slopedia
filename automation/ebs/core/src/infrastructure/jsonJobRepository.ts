...
export class JsonJobRepository implements JobRepository<Job, CreateJobInput> {
  ...
  async recoverInterruptedJobs(): Promise<Job[]> { return this.withLock(async () => { const data = await this.read(); const interrupted = data.jobs.filter((job) => ["running", "waiting_publish", "publishing"].includes(job.status)); const now = new Date().toISOString(); for (const job of interrupted) { job.status = "failed_review_required"; job.finishedAt = now; job.updatedAt = now; job.errorMessage = "Bot restarted while this job was active. Review or retry the job."; } if (interrupted.length) await this.write(data); return interrupted; }); }
