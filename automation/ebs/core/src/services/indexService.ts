...
export interface IndexManifest { generated_at: string; article_count: number; public_article_count: number; source_revision_hash: string; index_source_hashes: Record<string, string>; indexes: Record<string, string>; }
...
function analyzeLinks(articles: PublicArticle[]): LinkAnalysis {
  ...
  for (const { metadata, body } of articles) {
    ...
    for (const raw of extractInternalTargets(body)) { const ids = byTarget.get(normalizeTarget(raw)); if (!ids?.size) broken.push({ sourceId: metadata.id, target: raw, reason: "missing" }); else if (ids.size > 1) broken.push({ sourceId: metadata.id, target: raw, reason: "ambiguous" }); else targets.add([...ids][0]); }
...
async function buildManifest(dir: string, articleCount: number, articles: PublicArticle[], sourceHash: string, indexSourceHashes: Record<string, string>): Promise<IndexManifest> { const names = ["search-index.json", "category-index.json", "related.json", "backlink-index.json", "sitemap.xml"]; const indexes: Record<string, string> = {}; for (const name of names) try { indexes[name.replace(/\.(json|xml)$/, "")] = hash(await fs.readFile(path.join(dir, name))); } catch { /* partial rebuild */ } const mocFiles = await fs.readdir(path.join(dir, "moc")).catch(() => []); const mocBodies = await Promise.all(mocFiles.sort().map((file) => fs.readFile(path.join(dir, "moc", file)))); if (mocBodies.length) indexes.moc = hash(Buffer.concat(mocBodies)); return { generated_at: articles.map((a) => a.metadata.updatedAt).sort().at(-1) ?? "1970-01-01T00:00:00.000Z", article_count: articleCount, public_article_count: articles.length, source_revision_hash: sourceHash, index_source_hashes: sortRecord(indexSourceHashes), indexes: sortRecord(indexes) }; }
