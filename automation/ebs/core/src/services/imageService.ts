...
export class ImageService {
  ...
  async migrateArticle(article: ArticleMetadata, dryRun = true): Promise<ImageMigrationResult> {
    ...
    try {
      ...
      await fs.rename(temporary, target); const meta = await this.metadata(target, article.image?.alt ?? `「${article.title}」に関する図解`); await this.repository.update(article.id, { image: { ...meta, path: targetRelative, source: { type: "migrated" } } }); return { articleId: article.id, source: relative(this.vaultRoot, source), target: targetRelative, status: "migrated", message: "Converted and attached canonical WebP" };
  ...
  async inspect(image: ArticleImage): Promise<{ issue?: string; metadata?: ArticleImage }> { try { const file = safe(this.vaultRoot, image.path); const meta = await this.metadata(file, image.alt); if (!image.path.toLowerCase().endsWith(".webp")) return { issue: "non_webp", metadata: meta }; if (!image.alt.trim()) return { issue: "missing_alt", metadata: meta }; if (meta.bytes! > this.policy.maxBytes) return { issue: "oversized", metadata: meta }; if (!meta.width || !meta.height || Math.abs(meta.width / meta.height - 16 / 9) > .15) return { issue: "bad_aspect", metadata: meta }; if (image.sha256 && image.sha256 !== meta.sha256) return { issue: "hash_mismatch", metadata: meta }; return { metadata: meta }; } catch { return { issue: "corrupt_or_missing" }; } }
